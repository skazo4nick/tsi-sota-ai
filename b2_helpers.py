...
def upload_file_to_b2(b2_api, bucket_name, local_file_path, b2_file_path):
    ...
    try:
        ...
        print(f"File '{local_file_path}' uploaded to '{b2_file_path}' in bucket '{bucket_name}'.")
        return uploaded_file
    except Exception as e:
        print(f"Error uploading file '{local_file_path}' to '{b2_file_path}': {e}")
        raise
...
def download_file_from_b2(b2_api, bucket_name, b2_file_path, local_file_path):
    ...
    try:
        ...
        print(f"File '{b2_file_path}' downloaded to '{local_file_path}'.")
        return downloaded_file
    except Exception as e:
        print(f"Error downloading file '{b2_file_path}' to '{local_file_path}': {e}")
        raise
...
def delete_b2_file(b2_api, bucket_name, b2_file_path):
    ...
    try:
        ...
        print(f"File '{b2_file_path}' deleted from bucket '{bucket_name}'.")
        return deleted_file
    except Exception as e:
        print(f"Error deleting file '{b2_file_path}': {e}")
        raise
...
def get_b2_file_metadata(b2_api, bucket_name, b2_file_path):
    ...
    try:
        ...
        print(f"Error getting metadata for file '{b2_file_path}': {e}")
        raise
...
def sync_b2_with_local(b2_api, bucket_name, local_dir_path, b2_prefix):
    ...
    try:
        ...
        print(f"Successfully synchronized '{local_dir_path}' with bucket '{bucket_name}' at prefix '{b2_prefix}'.")
    except Exception as e:
        print(f"Error synchronizing '{local_dir_path}' with bucket '{bucket_name}': {e}")
        raise
